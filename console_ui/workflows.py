...
"""
            ...
            # Step 4.6: Generate B-roll Table using AI Agent
            broll_table = None  # Initialize outside try block
            try:
                print(f"\n📊 STEP 4.6: B-Roll Search Terms Table Generation")
                print("-" * 55)
                ...
                try:
                    ...
                    print("� Script creation will continue without B-roll table")
                    raise

                broll_agent = ScriptBRollAgentClient()
                print("✅ B-roll agent initialized")

                print("🚀 Generating B-roll table from script...")
                broll_result = broll_agent.generate_broll_table(
                ...
                if broll_result.get("success", False):
                    ...
                    print(
                        f"✅ B-roll table generated ({len(broll_table)} characters)")

                    # Append B-roll table to the main script markdown file
                    print("📝 Appending B-roll table to script markdown file...")
                    broll_section = f"\n\n{'=' * 80}\n"
                    ...
                    with open(script_file, "a", encoding="utf-8") as f:
                        f.write(broll_section)
                    print(f"✅ B-roll table appended to: {script_file}")
                else:
                    error_msg = broll_result.get("error", "Unknown error")
                    print(f"⚠️ B-roll table generation failed: {error_msg}")

            except Exception as broll_gen_error:
                print(f"❌ B-roll table generation error: {broll_gen_error}")
                print("� Script creation will continue without B-roll table")
            ...
            # Step 6: Generate B-Roll Search Terms Table (append to script)
            try:
                print(f"\n🎬 STEP 6: B-ROLL SEARCH TERMS GENERATION")
                print("-" * 55)
                ...
                print("🚀 Analyzing script for B-roll search terms...")
                broll_result = broll_agent.generate_broll_search_terms(
                    script_content=final_script_content,
                    script_title=script_topic,
                    max_terms=30,
                    timeout=180
                ...
                if broll_result.get("success", False):
                    search_terms = broll_result.get("search_terms", [])
                    raw_response = broll_result.get("raw_response", "")

                    print(
                        f"✅ Generated {len(search_terms)} B-roll search terms")
                    ...
                    for line in raw_response.split("\n"):
                        ...
                        if line and (line.endswith(":") or "**" in line):
                            if len(line) < 50:
                                potential_cat = line.replace(
                                    "**", "").replace(":", "").strip()
                                if potential_cat and not potential_cat.lower(
                                ).startswith(("column", "section", "quick")):
                                    current_category = potential_cat
                        else:
                            for term in search_terms:
                                if (term.lower() in line.lower() and
                                        term not in categories_map):
                                    categories_map[term] = current_category

                    # Create B-roll table markdown
                    broll_table = f"\n\n{'=' * 60}\n"
                    broll_table += "🎬 B-ROLL SEARCH TERMS REFERENCE\n"
                    broll_table += "=" * 60 + "\n\n"
                    broll_table += "| # | Category | Search Term |\n"
                    broll_table += "|---|----------|-------------|\n"

                    for idx, term in enumerate(search_terms, 1):
                        category = categories_map.get(term, "General")
                        broll_table += f"| {idx} | {category} | {term} |\n"

                    broll_table += f"\n**Total Terms:** {len(search_terms)}\n"

                    # Append B-roll table to the script file
                    with open(script_file, "a", encoding="utf-8") as f:
                        f.write(broll_table)

                    print(f"✅ B-roll search terms table appended to script")
                    print(
                        f"   📊 {len(search_terms)} terms organized by category")

                else:
                    print(f"⚠️ B-roll term generation failed")

            except Exception as broll_error:
                print(f"⚠️ B-roll term generation error: {broll_error}")
                print("📝 Script still saved without B-roll terms")
            ...
            print("🎬 B-roll search terms table appended to script!")
...
async def run_script_polisher_workflow():
    ...
    try:
        ...
        # Step 1.7.5: Generate B-roll Table using AI Agent
        broll_table = None  # Initialize outside try block
        try:
            print(f"\n📊 STEP 1.7.5: B-ROLL SEARCH TERMS TABLE GENERATION")
            print("-" * 55)
            ...
            try:
                ...
                print(f"⚠️ B-roll agent not available: {import_err}")
                print("📝 Script polishing will continue without B-roll table")
                raise

            broll_agent = ScriptBRollAgentClient()
            print("✅ B-roll agent initialized")

            print("🚀 Generating B-roll table from polished script...")
            broll_result = broll_agent.generate_broll_table(
            ...
            if broll_result.get("success", False):
                ...
                # Append B-roll table to the polished script markdown file
                print("📝 Appending B-roll table to polished script...")
                broll_section = f"\n\n{'=' * 80}\n"
                broll_section += "# 🎬 B-ROLL SEARCH TERMS TABLE\n"
                broll_section += f"{'=' * 80}\n\n"
                broll_section += broll_table
                ...
                current_content = polished_path.read_text(encoding="utf-8")
                updated_content = current_content + broll_section
                polished_path.write_text(updated_content, encoding="utf-8")
                print(f"✅ B-roll table appended to: {polished_path}")
            else:
                error_msg = broll_result.get("error", "Unknown error")
                print(f"⚠️ B-roll table generation failed: {error_msg}")
        ...
        try:
            ...
            if demo_result.get("success", False):
                ...
                # Step 3: Generate B-Roll Search Terms Table (append to script)
                try:
                    print("\n🎬 STEP 3: B-ROLL SEARCH TERMS GENERATION")
                    print("-" * 55)
                    ...
                    print("🚀 Analyzing polished script for B-roll terms...")
                    broll_result = broll_agent.generate_broll_search_terms(
                        script_content=polished_script,
                        script_title=script_title,
                        max_terms=30,
                    ...
                    if broll_result.get("success", False):
                        ...
                        # Append to polished script file
                        with open(polished_path, "a", encoding="utf-8") as f:
                            f.write(broll_table)

                        print("✅ B-roll table appended to polished script")
                        print(f"   📊 {len(search_terms)} terms by category")

                    else:
                        print("⚠️ B-roll term generation failed")

                except Exception as broll_error:
                    print(f"⚠️ B-roll term error: {broll_error}")
                    print("📝 Polished script saved without B-roll terms")
...
async def run_broll_downloader_workflow():
    """
    B-Roll Video Downloader workflow using AI agent to generate search terms
    Downloads 3 videos per search term from Pexels and Pixabay
    """
    print("🎬 B-ROLL VIDEO DOWNLOADER")
    print("=" * 60)
    print("This workflow will:")
    print("1. 🤖 Use AI agent to analyze your script and generate B-roll search terms")
    print("2. 📥 Download 3 videos per search term from Pexels + Pixabay")
    print("3. 📁 Save videos to ~/Dev/Videos/bRoll/[script_title]/")
    print()
    ...
    try:
        ...
        print("📝 SCRIPT INPUT")
        print("-" * 30)
        print("Provide your script for B-roll analysis:")
        print("  1. Load from file")
        print("  2. Paste script content")
        ...
        if input_choice == "1":
            ...
            if scripts_dir.exists():
                ...
                if script_files:
                    ...
                    try:
                        file_choice = input(
                            f"\n👆 Select file (1-{min(len(script_files), 10)}): "
                        ).strip()
                        file_idx = int(file_choice) - 1
                        if 0 <= file_idx < len(script_files):
                            selected_file = script_files[file_idx]
                            ...
                            script_title = extract_script_title(script_content)
                            if not script_title or len(script_title) < 3:
                                script_title = selected_file.stem
                            print(
                                f"✅ Loaded script: {selected_file.name} "
                                f"({len(script_content)} characters)"
                            )
            ...
            # Get script title
            try:
                script_title = input("📝 Enter script title: ").strip()
            except EOFError:
                script_title = "Untitled_Script"
            if not script_title:
                script_title = extract_script_title(script_content)
                if not script_title or len(script_title) < 3:
                    script_title = "Untitled_Script"
        ...
        if not script_title.strip():
            script_title = "Untitled_Script"

        print(f"\n📋 Script title: {script_title}")
        print(f"📊 Script length: {len(script_content)} characters")

        # Step 1: Generate B-roll search terms using AI agent
        print("\n🤖 STEP 1: Generating B-roll Search Terms with AI Agent")
        print("-" * 60)

        try:
            from linedrive_azure.agents import ScriptBRollAgentClient

            print("🔧 Initializing Script-bRoll-Agent...")
            broll_agent = ScriptBRollAgentClient()
            print("✅ Agent initialized successfully")

            print("\n🚀 Analyzing script and generating search terms...")
            print("   (This may take 60-90 seconds for comprehensive analysis)")

            result = broll_agent.generate_broll_search_terms(
                script_content=script_content,
                script_title=script_title,
                max_terms=30,  # Request ~30 terms, agent may provide more
                timeout=180     # Increased timeout for comprehensive analysis
            )

            if not result.get("success", False):
                error_msg = result.get("error", "Unknown error")
                print(f"❌ Agent failed to generate search terms: {error_msg}")
                return
            ...
            if not search_terms:
                print("❌ No search terms generated. Please try again.")
                return

            print(f"\n✅ Generated {len(search_terms)} B-roll search terms:")
            for i, term in enumerate(search_terms, 1):
                print(f"   {i}. {term}")

            # Parse categories from raw response for markdown table
            categories_map = {}  # {term: category}
            current_category = "General"

            for line in raw_response.split("\n"):
                line = line.strip()
                # Detect category headers (e.g., "Technology:", "**Actions:**")
                if line and (line.endswith(":") or "**" in line) and len(line) < 50:
                    potential_category = line.replace(
                        "**", "").replace(":", "").strip()
                    if potential_category and not potential_category.lower().startswith(("column", "section", "quick", "table")):
                        current_category = potential_category
                # Map terms to current category
                else:
                    for term in search_terms:
                        if term.lower() in line.lower() and term not in categories_map:
                            categories_map[term] = current_category

        except Exception as e:
            print(f"❌ Error initializing B-roll agent: {e}")
            import traceback
            print(f"🔍 Error details: {traceback.format_exc()}")
            return

        # Step 2: Download videos for each search term
        print("\n📥 STEP 2: Downloading B-roll Videos")
        print("-" * 60)
        ...
        safe_title = sanitize_filename(script_title)
        script_broll_dir = broll_base_dir / safe_title
        script_broll_dir.mkdir(exist_ok=True)

        print(f"📁 Output directory: {script_broll_dir}")

        # Initialize video downloaders
        try:
            import sys
            sys.path.append("./tools/media")
            ...
            # Initialize Pexels
            pexels_downloader = PexelsVideoDownloader()
            pexels_downloader.output_dir = script_broll_dir
            pexels_downloader.metadata_file = script_broll_dir / \
                "pexels_video_metadata_log.json"
            pexels_downloader.metadata_log = pexels_downloader._load_metadata_log()
            print("   ✅ Pexels downloader ready")

            # Initialize Pixabay
            pixabay_downloader = PixabayVideoDownloader()
            pixabay_downloader.output_dir = script_broll_dir
            pixabay_downloader.metadata_file = script_broll_dir / \
                "pixabay_video_metadata_log.json"
            pixabay_downloader.metadata_log = pixabay_downloader._load_metadata_log()
            print("   ✅ Pixabay downloader ready")

        except ImportError as e:
            print(f"❌ Failed to import video downloaders: {e}")
            print(
                "   Make sure pexels_video_downloader.py and pixabay_video_downloader.py exist")
            return

        # Download videos for each search term
        downloaded_files = []
        ...
        for term_idx, search_term in enumerate(search_terms, 1):
            print(
                f"\n🎬 Search Term {term_idx}/{len(search_terms)}: '{search_term}'")
            print("-" * 60)
            ...
            try:
                ...
                videos = pexels_downloader.search_videos(
                    search_term, per_page=10)

                if videos:
                    print(f"   ✅ Found {len(videos)} Pexels videos")

                    for video in videos:
                        if term_downloads >= videos_per_term:
                            break

                        video_id = str(video.get("id", ""))
                        video_url = video.get("video_files", [{}])[
                            0].get("link", "")
                        unique_id = f"pexels_{video_id}_{video_url}"
                        ...
                        if unique_id not in downloaded_video_ids:
                            ...
                            import re
                            clean_term = re.sub(r'[^\w\s-]', '', search_term)
                            clean_term = re.sub(r'\s+', '_', clean_term)[:40]
                            filename_base = f"{term_idx:02d}_{clean_term}_Pexels"
                            ...
                            if downloaded_file and Path(downloaded_file).exists():
                                ...
                                file_size = Path(
                                    downloaded_file).stat().st_size / (1024 * 1024)
                                print(
                                    f"   ✅ [{term_downloads}/{videos_per_term}] Pexels: {Path(downloaded_file).name} ({file_size:.1f} MB)")
                else:
                    print(f"   ⚠️ No Pexels videos found")
            ...
            if term_downloads < videos_per_term:
                try:
                    print(
                        f"   🔍 Searching Pixabay (need {videos_per_term - term_downloads} more)...")
                    videos = pixabay_downloader.search_videos(
                        search_term, per_page=10)

                    if videos:
                        print(f"   ✅ Found {len(videos)} Pixabay videos")
                        ...
                        for video in videos:
                            ...
                            video_id = str(video.get("id", ""))
                            video_url = video.get("videos", {}).get(
                                "large", {}).get("url", "")
                            unique_id = f"pixabay_{video_id}_{video_url}"
                            ...
                            if unique_id not in downloaded_video_ids:
                                ...
                                clean_term = re.sub(
                                    r'\s+', '_', clean_term)[:40]
                                filename_base = f"{term_idx:02d}_{clean_term}_Pixabay"
                                ...
                                if downloaded_file and Path(downloaded_file).exists():
                                    ...
                                    file_size = Path(
                                        downloaded_file).stat().st_size / (1024 * 1024)
                                    print(
                                        f"   ✅ [{term_downloads}/{videos_per_term}] Pixabay: {Path(downloaded_file).name} ({file_size:.1f} MB)")
                    else:
                        print(f"   ⚠️ No Pixabay videos found")

                except Exception as e:
                    print(f"   ❌ Pixabay error: {e}")

            # Summary for this term
            if term_downloads > 0:
                print(
                    f"   📊 Downloaded {term_downloads}/{videos_per_term} videos for this term")
            else:
                print(f"   ⚠️ No videos downloaded for this term")

        # Final summary
        print("\n" + "=" * 60)
        print("🎉 B-ROLL DOWNLOAD COMPLETE!")
        print("=" * 60)
        print(f"📋 Script: {script_title}")
        print(f"🎬 Search terms: {len(search_terms)}")
        print(f"📥 Videos downloaded: {len(downloaded_files)}")
        print(f"📁 Location: {script_broll_dir}")

        if downloaded_files:
            total_size = sum(
                Path(f).stat().st_size for f in downloaded_files) / (1024 * 1024)
            print(f"💾 Total size: {total_size:.1f} MB")
            print(f"\n📋 Downloaded files:")
            for file_path in downloaded_files:
                file_size = Path(file_path).stat().st_size / (1024 * 1024)
                source = "Pexels" if "Pexels" in Path(
                    file_path).name else "Pixabay"
                print(
                    f"   • [{source}] {Path(file_path).name} ({file_size:.1f} MB)")

        # Create markdown table with categories and search terms
        print("\n📝 Creating search terms reference table...")
        markdown_content = f"""# B-Roll Search Terms Reference
**Script:** {script_title}  
**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}  
**Total Terms:** {len(search_terms)}  
**Videos Downloaded:** {len(downloaded_files)}
...
"""
        ...
        for idx, term in enumerate(search_terms, 1):
            category = categories_map.get(term, "General")
            markdown_content += f"| {idx} | {category} | {term} |\n"

        markdown_content += f"""
## Summary

- **Total Categories:** {len(set(categories_map.values()))}
- **Total Search Terms:** {len(search_terms)}
- **Videos Downloaded:** {len(downloaded_files)}
- **Storage Location:** `{script_broll_dir}`
...
"""
        ...
        category_counts = {}
        for term in search_terms:
            cat = categories_map.get(term, "General")
            category_counts[cat] = category_counts.get(cat, 0) + 1

        for category, count in sorted(category_counts.items(), key=lambda x: x[1], reverse=True):
            markdown_content += f"- **{category}:** {count} terms\n"

        # Save markdown file
        markdown_file = script_broll_dir / "search_terms_reference.md"
        with open(markdown_file, "w", encoding="utf-8") as f:
            f.write(markdown_content)

        print(f"   ✅ Saved: {markdown_file.name}")
        print(
            f"   📄 Contains {len(search_terms)} search terms organized by category")

    except Exception as e:
        print(f"❌ B-roll workflow error: {e}")
        import traceback
        print(f"🔍 Error details: {traceback.format_exc()}")
    finally:
        try:
