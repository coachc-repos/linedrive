...
def run_scrapers_menu():
    ...
    while True:
        ...
        if choice == "0":
            ...
            subprocess.run([sys.executable, "console_ui/scrapers/perfect_game_menu.py"])
        elif choice == "2":
            subprocess.run([sys.executable, "console_ui/scrapers/url_scraper_menu.py"])
        elif choice == "3":
            subprocess.run(
                [sys.executable, "console_ui/scrapers/youtube_transcript_menu.py"]
            )
        elif choice == "4":
            subprocess.run([sys.executable, "console_ui/scrapers/batch_scraper_ui.py"])
        else:
...
def run_automation_menu():
    ...
    while True:
        ...
        try:
            choice = input("👆 Select automation tool (0-6): ").strip()
        except EOFError:
            choice = "0"
            print(f"   Using default (EOF): {choice}")
        ...
        if choice == "0":
            ...
            subprocess.run(
                [sys.executable, "console_ui/automation/autogen_tournament_ui.py"]
            )
        elif choice == "2":
            subprocess.run(
                [sys.executable, "console_ui/automation/batch_job_runner.py"]
            )
        elif choice == "3":
...
def run_social_media_menu():
    ...
    while True:
        ...
        if choice == "0":
            ...
            subprocess.run(
                [sys.executable, "console_ui/social_media/ai_tweet_generator.py"]
            )
        elif choice == "2":
            subprocess.run(
                [sys.executable, "console_ui/social_media/verify_social_media_ui.py"]
            )
        elif choice == "3":
            subprocess.run(
                [sys.executable, "console_ui/social_media/quick_tweet_ui.py"]
            )
        else:
